...
The four stage folders of the optimization pipeline, each holding its own `summary.txt` per dataset:

- `accessrefinery_bdd_reducer_20rs/` — `BR`, the pipeline with all three optimizations off.
- `accessrefinery_bdd_reducer_AR_20rs/` — `AR`, `BR` plus the reducer's essential-finding pre-filter.
- `accessrefinery_bdd_reducer_AM_20rs/` — `AM`, `AR` plus the miner's BFS early-exit and refinement DAG.
- `accessrefinery_bdd_reducer_AI_20rs/` — `AI`, `AM` plus the incremental EC engine (all defaults).
...
> Note on the incremental column: its value is the cost of exactly one `computeLabels()` call — the call that absorbs the new label — not the cost of the whole preprocessing pipeline that the column measures elsewhere in this artifact. It is taken from a timing-instrumented build: the factory is first primed by building the policy from its label-free form, then the augmented policy is built and the single `computeLabels()` call that sees the new label is timed. Re-running the stage reproduces the run but not this column; the archived `AI_20rs/RW/summary.txt` is where these numbers come from.
