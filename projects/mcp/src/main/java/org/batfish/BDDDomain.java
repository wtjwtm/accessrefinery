...
public final class BDDDomain<T> {
  ...
  private @Nonnull BDDFactory _factory;
  ...
  /**
   * Adds a value to this domain without changing the underlying BDD variable assignment.
   *
   * <p>This is only possible when the number of values does not exceed the current bit encoding.
   * If adding the value would require more bits (i.e., the value count passes a 2^k boundary),
   * an {@link IllegalStateException} is thrown and the caller must rebuild the domain instead.
   *
   * @param value the value to add
   * @throws IllegalStateException if adding the value would require more BDD variables
   */
  public void addValue(T value) {
    ...
    if (newBits > oldBits) {
      throw new IllegalStateException(
          "Cannot add value " + value + " to domain: would require " + newBits
