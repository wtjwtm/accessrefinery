...
public class AccessRefinery {
    ...
    private HashSet<MCPIntent> reducingIntents(MCPPolicy mcpPolicy, HashSet<MCPIntent> mcpFinding, ResultsAnalyzer analyzer) throws IOException {
        ...
        if (Parameter.isOptAR()) {
            // ── Phase 1: Identify essential findings via BDD unique-coverage check ──
            candidates = new ArrayList<>();
            remainingPolicy = essentialPreFilter(findings, policySpace, essential, candidates);
            analyzer.addRRIOperationsTime();
        } else {
            // -Dopt.ar=false: pre-optimization path. Every finding is a candidate and
            ...
            Parameter.LOGGER.info("[5/6]  essential pre-filter disabled: "
    ...
    /**
     * Phase 1 of the reducer: partitions the findings into those that uniquely cover
     * some part of the policy space ({@code essential}, never worth solving for) and
     * the rest ({@code candidates}), using a BDD prefix/suffix scan so that each
     * finding's coverage is compared against all others in one pass.
     *
     * @param findings the non-empty findings to partition
     * @param policySpace the symbolic policy space being covered
     * @param essential populated with the uniquely-covering findings
     * @param candidates populated with the findings left for the EC/ILP set cover
     * @return the policy space not yet covered by the essential findings
     */
    private MCPBitVector essentialPreFilter(List<MCPIntent> findings, MCPBitVector policySpace,
                                            HashSet<MCPIntent> essential, List<MCPIntent> candidates) {
        int n = findings.size();
    ...
    private HashSet<MCPIntent> miningIntents(MCPPolicy mcpPolicy, MCPIntent rootFinding, TimeMeasure time, ResultsAnalyzer analyzer) throws IOException {
        ...
        while(!queue.isEmpty()) {
            ...
            if (Parameter.isOptAM()) {
                // -Dopt.am=false: pre-optimization path. A finding disjoint from the
            ...
            if (!Parameter.isOptAM() && Parameter.isTimeLog) {
                time.stopMeasurement("Operation");
                TimeMeasure.appendValuesToFile(Parameter.timeLog, time);
            }
