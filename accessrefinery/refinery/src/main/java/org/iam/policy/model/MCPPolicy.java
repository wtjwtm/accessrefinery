...
public class MCPPolicy extends Policy implements MCPVar {
    ...
    public void initialMCPFactory() {
        ...
        // -Dopt.ai=false forces the batch engine regardless, since the incremental
        // engine is the AI stage's optimization.
        String v = System.getProperty("mcp.incremental");
        boolean incremental = Parameter.isOptAI()
                && ((v == null) || !(v.equalsIgnoreCase("false") || v.equals("0")));

        if (incremental) {
            // -Dinc.log=true reports the FINAL incremental update() cost (the last
            // statement), i.e. the marginal EC cost of the last statement on the state
            // built by the previous 1..K-1. Off by default: one line per policy per
            // round would otherwise flood stdout on the full RW corpus.
            boolean incLog = Boolean.parseBoolean(System.getProperty("inc.log", "false"));
            long[] _lastSecNs = {0};
            int[] _cntSec = {0};
            allowMCPStatements.forEach(statement -> {
                statement.initialMCPFactory();
                long _t0 = incLog ? System.nanoTime() : 0L;
                _mcpFactory.updates();
                if (incLog) {
