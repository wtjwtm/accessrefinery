...
public class MCPFactory extends MCPLabels {
    ...
    static {
        // -Dmcp.stats=true dumps the update statistics at JVM exit. Off by default so
        // that the experiment runner's stdout stays readable; the same counters remain
        // available to callers through getUpdateStats().
        if (Boolean.parseBoolean(System.getProperty("mcp.stats", "false"))) {
